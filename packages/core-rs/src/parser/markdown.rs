...
use self::data_image::{DataImageError, image_fallback_text, parse_data_uri};
use self::state::{
    Container, InlineContext, append_fallback_separator, code_block_language, current_list_depth,
    current_table_mut, empty_to_none, flatten_inline_text, heading_level, push_inline_merged,
    table_alignment, trim_trailing_fallback_whitespace, unsupported_block_feature,
...
impl MarkdownParser {
    ...
    fn handle_start(
        ...
        match tag {
            ...
            Tag::Table(alignments) => containers.push(Container::Table(state::TableBuilder {
                aligns: alignments.into_iter().map(table_alignment).collect(),
                headers: Vec::new(),
                rows: Vec::new(),
            })),
            Tag::TableHead => containers.push(Container::TableHead(Vec::new())),
            Tag::TableRow => containers.push(Container::TableRow(state::TableRowBuilder {
                cells: Vec::new(),
            })),
