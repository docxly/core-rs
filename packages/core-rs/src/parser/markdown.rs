...
use self::state::{
    Container, InlineContext, append_fallback_separator, code_block_language,
    current_list_depth, current_table_mut, empty_to_none, flatten_inline_text, heading_level,
    push_inline_merged, table_alignment, trim_trailing_fallback_whitespace,
    unsupported_block_feature, unsupported_block_nesting_end, unsupported_block_nesting_tag,
};
...
impl MarkdownParser {
    ...
    fn handle_start(
        ...
        match tag {
            ...
            Tag::TableRow => {
                containers.push(Container::TableRow(state::TableRowBuilder { cells: Vec::new() }))
            }
