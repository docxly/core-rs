# 한글 문서 파일 구조 5.0 - data records and history
...
- Physical PDF pages: 21-70
- Extraction method: `pdftotext -layout`
- Notes: Printed section: `4. 데이터 레코드` and `4.4 문서 이력 관리`. Covers DocInfo/body records, control/object records, section definitions, and document history management.
...
한글 문서 파일 형식 5.0
     ...
     4.1. 데이터 레코드 구조
       ...
       레코드의 헤더에는 데이터 확장에 대비한 정보를 가지고 있다. 따라서 이후에 한글의 기능이 확장되어
       레코드에 데이터가 추가되는 경우에도 하위 버전의 한글이 상위 버전의 한글 문서를 읽을 수 있도록
       하위 호환성이 보장된다.
...
COLORREF             4       무늬색
  INT32              4       무늬 종류(표 29 참조)
    ...
    INT16            2       그러데이션 번짐 정도(0 -100)
                             그러데이션의 색 수(num). (워디안/한글2002/SE에서는
    INT16            2
                             항상 2이다.)
     ...
     4.2.8. 문단 번호
       Tag ID : HWPTAG_NUMBERING
          ...
          WORD           2       번호 형식 길이 (len)
                                 ...
                                 문자열 내 특정 문자에 제어코드(^)를
                                 붙임으로써 한글에서 표시되는 번호 문단                수준(1～7)
          WCHAR                  머리의 형식을 제어한다.
                       2×len                                          각 레벨에 해당하는
         array[len]               ^n : 레벨 경로를 표시한다.(예:
          ...
          WCHAR                  문자열 내 특정 문자에 제어코드(^)를
                       2×len                                          각 레벨에 해당하는
         array[len]              붙임으로써 한글에서 표시되는 번호 문단
...
4.3.4. 문단의 레이아웃
  ...
  Tag ID : HWPTAG_PARA_LINE_SEG
       ...
       UINT32          4      -bit 18 : 줄의 마지막 세그먼트인지 여부
...
한글 문서 파일 형식 5.0
     ...
     4.3.9.3. 한글 수식 개체
       ...
       *한글 수식 스크립트는 EQN 스크립트 호환이며 자세한 스펙은 뒤에 추가
...
한글 문서 파일 형식 5.0
     ...
     4.3.10. 개체 이외의 컨트롤
           ...
           7           감추기              MAKE_4CHID('p', 'g', 'h', 'd')           부터 시작하며 ctrlid로
                                                                                 각 개체를 확인 할 수
...
4.3.10.1.2. 각주/미주 모양
   Tag ID : HWPTAG_FOOTNOTE_SHAPE
    ...
    bit 0～7     0x80, 0x81은   9  동그라미 쳐진 가,나,다
                ...
                한 페이지         1  (각주인 경우) 통단으로 배열
                내에서 각주를
    bit 8～9                   2  (각주인 경우) 가장 오른쪽 단에 배열
                다단에
                위치시킬 방법       0  (미주인 경우) 문서의 마지막
                              1  (미주인 경우) 구역의 마지막
                              0  앞 구역에 이어서
...
한글 문서 파일 형식 5.0
     ...
     4.4.1. 문서 이력 관리란
       ...
       한글 메뉴의 “파일-문서 이력 관리”에서 표시 및 생성되는 문서의 이력 정보를 저장하는 장소이다.(한글
       2005에서 한글 2007까지는 ’버전 비교(파일-버전 비교)’라는 이름으로 ’문서 이력 관리 정보’ 기능을
       제공하였다.)
